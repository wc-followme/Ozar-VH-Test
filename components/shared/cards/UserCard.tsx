...
import { IconDotsVertical, IconPhoneCalling } from "@tabler/icons-react";
    ...
    <div className="bg-[var(--white-background)] rounded-[12px] border border-[var(--border-dark)] p-[10px] hover:shadow-lg transition-shadow duration-200">
      ...
      <div className="flex items-start gap-4 mb-2">
        <Avatar className="w-20 h-20 rounded-[10px]">
          <AvatarImage src={image} alt={name} className="rounded-[10px] object-cover" />
        ...
        <div className="flex-1 min-w-0">
          {/* User Info */}
          <div className="mb-1">
            <h3 className="font-bold text-[var(--text)] truncate text-base">{name}</h3>
            <p className="text-[12px] font-medium text-[var(--text-dark)]">{role}</p>
          </div>
...
/>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="bg-[var(--white-background)] border border-[var(--border-dark)] shadow-[0px_2px_8px_0px_#0000001A] rounded-[8px]">
      ...
      <div className="flex items-center justify-between bg-[var(--border-light)] rounded-[30px] py-2 px-3">
        <span className="text-[12px] font-medium text-[var(--text-dark)]">Enable</span>
        <Switch
            checked={status}
